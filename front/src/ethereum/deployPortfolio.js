...
const deploy = async (minimal,daysForStart) => {
  const accounts = await web3.eth.getAccounts();
  const manager = accounts[0];
  const site = "0xdc42E3Ba842781923d2b51816da93Ab95b25Ca2b";

  console.log("Attempting to deploy from account", manager);
  const inbox = await new web3.eth.Contract(abi)
    .deploy({ data: evm.bytecode.object, arguments: [minimal,daysForStart,site,manager] })
