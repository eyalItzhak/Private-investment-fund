...
contract campaign {
    ...
    modifier WorkPeriodSetUp() { //for cheak we set up || for cheak and move for workpriod time
        require(
            (canBuy == false)
        );
        _;
    ...
    modifier EndPeriod() {
        require((endTheContract_numOfVote>=numStakeholders/2)||canBuy == false);
        _;
    }
    ...
    function investorBalanc () public view returns (uint) {
     return address(this).balance*(1-(manager_fee/1000)) ;
    }
    ...
    function startContract() public WorkPeriodSetUp {
        canBuy = true;
        uint totalVal = investorBalanc () ;
        uint p_val = totalVal / 1000;
        uint ammunt;
        for (uint i = 0; i < Investors.length; i++) {
            address temp_investor = Investors[i];
            ammunt = Investment[temp_investor];
            ownPercent[temp_investor] = (p_val * ammunt) / totalVal;
        }
        setStakeholders();
    }
    ...
    function setStakeholders() private {
        ...
        min = Investment[temp];
        value_temp[0] = min;
        for (uint i = 1; i < numStakeholders - 1; i++) {
        ...
        // for (uint i = numStakeholders - 1; i < Investors.length; i++) {
        ...
        //         min = temp_val;
        //         pos = i;
        //         for (uint j = 0; j < numStakeholders - 1; j++) {
        //             if (Investment[address_temp[j]] < min) {
        //                 min = temp_val;
        //                 pos = i;
        //             }
        //         }
    ...
    function leave_contract(address leave_address, uint yield) //get the total value of the portfolios (yield)
        public
        workPeriod
        onlySite
        returns (bool)
    {
        ...
        uint value = myPercent * yield * (penelty / 1000);
    ...
    function PayDayVoted() public workPeriod onlyStakeholders {
     ...
     function payDay() public EndPeriod  {
          for (uint i = 0 ; i < Investors.length; i++) {
              address investor= Investors[i];
              uint balance = investorBalanc ();
              if(Stakeholders_endTheContract[investor]==true){
                  uint myPercent = ownPercent[investor];
                  uint value = (myPercent/ 1000)*balance;
                  payable(investor).transfer(value);
              }
          }
           payable(manager).transfer(address(this).balance); //all what left is his manger share..
     }
}
